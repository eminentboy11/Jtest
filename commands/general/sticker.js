...
module.exports = {
  ...
  async execute(sock, msg, args, extra) {
    // System ffmpeg is a hard requirement in this edition (no ffmpeg-static:
    // ~70 MB on disk and ~50 MB RSS, which halves bots-per-VPS). Fail politely
    // instead of spawning a doomed child process.
    if (!ffmpegPath.hasFfmpeg()) {
      await sock.sendMessage(extra?.from || msg.key.remoteJid, {
        text: '❌ Sticker conversion needs ffmpeg on the server.\nInstall it with `apt install ffmpeg`, then try again.',
      }, { quoted: msg });
      return;
    }
