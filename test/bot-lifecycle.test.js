...
test('duplicate sessionId is rejected while duplicate phone is allowed', () => {
  const registry = [{ phone: '2348165321909', sessionId: 'JTEST~abcd' }];
  assert.equal(
    sessionManager.addSessionEntry(registry, { phone: '2348165321909', sessionId: '' }).ok,
    true
  );
  assert.equal(
    sessionManager.addSessionEntry(registry, { phone: '2348000000000', sessionId: 'JTEST~abcd' }).reason,
    'duplicate-sessionId'
  );
});
...
test('pause and resume preserve the registry entry and do not mutate input', () => {
  const original = [{ phone: '2348165321909', sessionId: 'JTEST~abcd' }];
