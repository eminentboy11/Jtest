...
function createBotDatabase(options = {}) {
  const botId = String(options.botId || 'default');
  const DB_DIR = path.resolve(process.env.JUNE_DB_DIR || path.join(__dirname, 'database'));
  const LEGACY_DB_FILE = path.resolve(process.env.JUNE_DB_FILE || path.join(DB_DIR, 'june-ultra.db'));
  const DB_FILE = options.dbFile
    ? path.resolve(options.dbFile)
    : (botId === 'default' ? LEGACY_DB_FILE : path.join(DB_DIR, `june-${botId}.db`));
  const LEGACY_DB_BACKUP_FILE = path.resolve(process.env.JUNE_DB_BACKUP_FILE || path.join(DB_DIR, 'june-ultra.db.backup'));
  ...
  const DB_MAINTENANCE_INTERVAL_MS = positiveNumberEnv('JUNE_DB_MAINTENANCE_INTERVAL_MS', 60 * 60 * 1000, { minimum: 60 * 1000 });
  const ANTIDELETE_RETENTION_MS = positiveNumberEnv('JUNE_ANTIDELETE_RETENTION_HOURS', 48, { minimum: 0 }) * 60 * 60 * 1000;
  const ANTIDELETE_STATUS_RETENTION_MS = positiveNumberEnv('JUNE_ANTIDELETE_STATUS_RETENTION_HOURS', 24, { minimum: 0 }) * 60 * 60 * 1000;
  const ANTIDELETE_MAX_ROWS = Math.floor(positiveNumberEnv('JUNE_ANTIDELETE_MAX_ROWS', 500, { minimum: 0 }));
  const ANTIDELETE_STATUS_MAX_ROWS = Math.floor(positiveNumberEnv('JUNE_ANTIDELETE_STATUS_MAX_ROWS', 500, { minimum: 0 }));
  const AUTO_DOWNLOAD_STATUS_HISTORY_MAX_ROWS = Math.floor(positiveNumberEnv('JUNE_AUTODOWNLOAD_STATUS_HISTORY_MAX_ROWS', 500, { minimum: 50 }));
  const REMOTE_SYNC_INTERVAL_MS = positiveNumberEnv('JUNE_REMOTE_SYNC_INTERVAL_MS', 30 * 1000, { minimum: 5 * 1000 });
  const REMOTE_SYNC_MAX_ROWS = Math.floor(positiveNumberEnv('JUNE_REMOTE_SYNC_MAX_ROWS', 2000, { minimum: 100 }));
  const AUTH_MIRROR_DEBOUNCE_MS = Math.floor(positiveNumberEnv('JUNE_AUTH_MIRROR_DEBOUNCE_MS', 2000, { minimum: 500 }));
...
const instance = {
  ...
  getMenuImageSettings,
  setMenuImageData,
  clearMenuImageData,
  getAutoDownloadStatusSettings,
  setAutoDownloadStatusSettings,
  AUTO_DOWNLOAD_STATUS_TYPES,
  AUTO_DOWNLOAD_STATUS_DEFAULTS,
  markStatusDownloaded,
  hasStatusBeenDownloaded,
  getStatusDownloadLogs,
  clearStatusDownloadHistory,
  incrementStatusDownloadCount,
  getAntideleteMode,
  setAntideleteMode,
  ANTIDELETE_MODES,
  getAntieditMode,
  ...
  setAntideleteStatusEnabled,
  getAntiTagAdminsSettings,
  setAntiTagAdminsSettings,
  ANTITAGADMINS_ACTIONS,
  isAntiAllEnabled,
  setAntiAllEnabled,
  ...
  getAntideleteMessages,
  deleteAntideleteMessage,
  saveAntideleteStatus,
  getAntideleteStatuses,
  ...
  runIntegrityCheck,
  getDatabaseHealth,
  createBackup: () => createAtomicBackup('manual'),
  flushBackup,
  shutdownDatabase,
  markDatabaseDirty,
  ...
  getRemoteAuthMirrorStatus,
  scheduleRemoteAuthMirror,
  flushRemoteAuthMirror,
  mirrorRemoteAuthState,
  restoreRemoteAuthState,
  clearRemoteAuthState,
  restoreFromPostgres,
  restoreFromMongo,
  backfillRemote,
  resetDatabase,
  getPostgresStatus: pg.getStatus,
